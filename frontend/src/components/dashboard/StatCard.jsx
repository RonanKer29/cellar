...
const StatCard = ({ title, value, subtitle, icon: Icon, color = "purple" }) => {
  ...
  return (
    <>
      ...
      <div className="sm:hidden group relative overflow-hidden bg-white rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border-0">
        ...
        <div className="relative z-10 p-4">
          <div className="flex items-center justify-between">
            {/* Icône compacte */}
            <div className={`w-10 h-10 ${theme.iconBg} rounded-lg flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300`}>
      ...
      {/* Version desktop/tablette (existante) */}
      <div className="hidden sm:block group relative overflow-hidden bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 border-0">
        {/* Gradient de fond subtil */}
        <div className={`absolute inset-0 ${theme.lightBg} opacity-30`}></div>

        {/* Barre décorative en haut */}
        <div
          className={`absolute top-0 left-0 right-0 h-1 ${theme.gradient}`}
        ></div>
        ...
        <div className="relative z-10 p-6">
          <div className="flex items-start justify-between mb-4">
            ...
            <div
              className={`w-14 h-14 ${theme.iconBg} rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300`}
            >
            ...
            {/* Badge ou indicateur */}
            <div
              className={`px-2 py-1 rounded-full text-xs font-medium ${theme.iconBg} ${theme.iconColor}`}
            >
              Stats
            </div>
          </div>
          ...
          <div className="mb-3">
            <div className="text-4xl font-black text-gray-600 leading-none">
              {value}
            </div>
          </div>
