...
const StatsOverview = ({ bottles }) => {
  ...
  const totalValue = bottles.reduce(
    (sum, bottle) => sum + (bottle.price || 0) * bottle.quantity,
    0
  );
  ...
  const averageAge =
    ...
    vintages.reduce((sum, year) => sum + year, 0) / vintages.length;

  // Most expensive wine
  const mostExpensiveWine = bottles.reduce(
    (max, bottle) => ((bottle.price || 0) > (max.price || 0) ? bottle : max),
    bottles[0] || {}
  );
  ...
  const producerCounts = bottles.reduce((counts, bottle) => {
    if (bottle.productor) {
      counts[bottle.productor] =
        (counts[bottle.productor] || 0) + bottle.quantity;
    }
    return counts;
  }, {});
  const topProducer = Object.entries(producerCounts).sort(
    ([, a], [, b]) => b - a
  )[0];
  ...
  const mainStats = [
    ...
    },
    {
      ...
      title: "Millésimes",
      value:
        vintages.length > 0 ? `${oldestVintage} - ${newestVintage}` : "N/A",
  ...
  const detailedStats = [
    {
      ...
      subtitle: mostExpensiveWine.price
        ? `${mostExpensiveWine.price} €`
        : "Prix non défini",
      color: "yellow",
    },
    {
      ...
      subtitle: topColor
        ? `${topColor[1]} bouteilles (${Math.round(
            (topColor[1] / totalBottles) * 100
          )}%)`
        : "",
      color: "red",
    },
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border-0">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600">
              Analyses approfondies de vos bouteilles
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {detailedStats.map((stat, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-5 hover:shadow-md transition-all duration-300"
              >
                {/* Barre colorée */}
                <div
                ...
                <div className="flex items-start space-x-4">
                  <div
                    className={`${theme.iconBg} rounded-xl p-3 group-hover:scale-110 transition-transform duration-300`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p
                      className="text-lg font-black text-gray-800 mb-1 truncate"
                      title={stat.value}
                    >
                      {stat.value}
                    </p>
            ...
            <div className="space-y-3">
              {Object.entries(regionCounts)
                ...
                .map(([region, count]) => (
                  ...
                  >
                    <span
                      className="text-gray-700 font-medium truncate flex-1 mr-2"
                      title={region}
                    >
                      {region}
                    </span>
