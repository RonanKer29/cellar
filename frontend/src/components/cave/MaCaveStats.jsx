...
const MaCaveStats = ({ bottles }) => {
  ...
  const averagePrice =
    bottles.length > 0
      ? totalValue / bottles.reduce((sum, bottle) => sum + bottle.quantity, 0)
      : 0;
  const oldestWine = bottles.reduce(
    (oldest, bottle) =>
      !oldest || bottle.year < oldest.year ? bottle : oldest,
    null
  );
  const newestWine = bottles.reduce(
    (newest, bottle) =>
      !newest || bottle.year > newest.year ? bottle : newest,
    null
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      title: "Millésimes",
      value:
        oldestWine && newestWine
          ? `${oldestWine.year} - ${newestWine.year}`
          : "-",
      subtitle: `Étendue: ${
        newestWine && oldestWine ? newestWine.year - oldestWine.year : 0
      } ans`,
      color: "blue",
    },
    {
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600">
                Distribution de vos vins par couleur
              </p>
            </div>
